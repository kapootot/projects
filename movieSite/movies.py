...
c = conn.cursor()
#create = c.execute("CREATE TABLE moviedb (title_id INT NOT NULL PRIMARY KEY AUTO_INCREMENT, title VARCHAR(100) NOT NULL, title_year VARCHAR(10) NULL, downloaded VARCHAR(40) NOT NULL, imdb_rating VARCHAR(5) NULL, watched ENUM ('Y', 'N') NOT NULL, subtitles ENUM ('Y', 'N') NOT NULL,updated TIMESTAMP, UNIQUE (title, imdb_rating));")
...
def concat(finalInput):
    ...
    conn.commit()
    conn.close()
    print "[SUCCESS] Movie DB refreshed"
